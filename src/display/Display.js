...
const Display = ({ closed, locked }) => {
  ...
  return (
    <div data-testid='display' className="display panel">
      <div data-testid='lockedStatus' className={lockedClass}>{locked ? 'Locked' : 'Unlocked'}</div>
      <div data-testid='closedStatus' className={closedClass}>{closed ? 'Closed' : 'Open'}</div>
    </div>
  );
};
